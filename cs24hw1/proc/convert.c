...
uint32_t convert(const char *bits) {
    ...
    /* Iterate over the '0' and '1' characters in the input array, 
    * get the  actual value represented by that and add to sum of string of bits */
    int j = 0;
    int count = 0;
    double sum = 0;
    while (bits[j] != '\0'){
    ...
    for(int i = 0; i < count; i++){
       if (bits[i] =='1'){
            sum +=  pow((double)2, (double)count - 1 - i);
       }
    }
    value = (uint32_t)sum;
    return value;
